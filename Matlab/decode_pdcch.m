function peak_out=decode_pdcch(peak,tfg)
...
% Channel estimation
ce_tfg=NaN(n_ofdm,nSC,4);
[ce_tfg(:,:,1) np0]=chan_est_20M(peak,tfg,0, nRB);
[ce_tfg(:,:,2) np1]=chan_est_20M(peak,tfg,1, nRB);
[ce_tfg(:,:,3) np2]=chan_est_20M(peak,tfg,2, nRB);
[ce_tfg(:,:,4) np3]=chan_est_20M(peak,tfg,3, nRB);
...
%for frame_timing_guess=3
  ...
  % Extract symbols and channel estimates for the PDCCH
  [pdcch_sym pdcch_ce]=pdcch_extract(peak,tfg_try,ce_try);

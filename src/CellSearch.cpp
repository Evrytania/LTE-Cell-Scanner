...
//    }
  ...
  if (verbosity>=1) {
    cout << "OpenCL LTE CellSearch v" << MAJOR_VERSION << "." << MINOR_VERSION << "." << PATCH_LEVEL << " (" << BUILD_TYPE << ") beginning. 1.0 to 1.1: OpenCL/TDD/ext-LNB added by Jiao Xianjun(putaoshu@gmail.com)" << endl;
...
int main(
  ...
  if ( dongle_used && freq_start!=9999e6) {
    ...
    int rtlsdr_exist = 0;
    if ( config_usb(sampling_carrier_twist,correction,device_index,freq_start,dev,fs_programmed) == 0 ) {
      rtlsdr_exist = 1;
    ...
    int hackrf_exist = 0;
    #ifdef HAVE_HACKRF
      if ( config_hackrf(sampling_carrier_twist,correction,device_index,freq_start,hackrf_dev,fs_programmed) == 0 ) {
        hackrf_exist = 1;
